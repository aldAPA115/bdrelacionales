<?php
    ...
    if($st){
        ...
        echo json_encode(["error" => "Actor actualizado correctamente"]);
        exit();
    }

    $st->bind_param("si", $first_name, $last_name, $actor_id);
    ...
    if($st->execute()){
        if($st->affected_rows > 0){
            ...
            echo json_encode(["error" => "No se encontró el actor con la id: $actor_id"]);
        }
    }else{
        http_response_code(500);
